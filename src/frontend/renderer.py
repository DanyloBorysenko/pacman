...
class Renderer:
    def __init__(
        ...
        self.is_mobile = True  # Feel free to link this to a platform detector!

        # Left side: D-Pad cluster anchor (
        # centered around x=100, y=WINDOW_HEIGHT - 100)
        dpad_x, dpad_y = 100, WINDOW_HEIGHT - 100
        btn_sz = 40  # Button size in pixels
        ...
        # (x=WINDOW_WIDTH - 100, y=WINDOW_HEIGHT - 100)
        self.touch_enter = pygame.Rect(WINDOW_WIDTH - 120, dpad_y - 30, 80, 60)
        self.touch_back = pygame.Rect(20, 20, 70, 40)

        # Create a single, persistent cached overlay surface
    ...
    def _pre_render_mobile_controls(self) -> None:
        ...
        # Draw the buttons onto our persistent self.controls_overlay
        pygame.draw.rect(
            self.controls_overlay, btn_color, self.touch_up, border_radius=5)
        pygame.draw.rect(
            self.controls_overlay, btn_color, self.touch_down, border_radius=5)
        pygame.draw.rect(
            self.controls_overlay, btn_color, self.touch_left, border_radius=5)
        pygame.draw.rect(
            self.controls_overlay, btn_color, self.touch_right,
            border_radius=5)
        pygame.draw.rect(
    ...
    def draw_mobile_controls(self, surface: pygame.Surface) -> None:
        if self.is_mobile:
            # Zero allocations! Just a super fast, hardware-accelerated copy.
