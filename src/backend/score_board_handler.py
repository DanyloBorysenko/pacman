...
class ScoreBoardHandler:
    ...
    def load_top_players(self) -> List[Dict[str, Any]]:
        ...
        If the file is absent or corrupted, initializes a clean, empty list profile structure.
        """
    ...
    def is_in_top_scored(self, score: int) -> bool:
        """
        Determines if a newly finished game score qualifies for the leaderboard.
        Returns True if the board has room (< 10 entries) OR if it beats the lowest top score.
        """
        if len(self.top_scores) < 10:
            return True
        # Assumes list is kept sorted; last entry is the lowest qualifying score
